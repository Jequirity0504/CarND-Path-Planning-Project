        ...
        if (event == "telemetry") {
           ...
           */
            ...
            for(int i = 0; i < sensor_fusion.size(); i++)
            {
                ...
                double vEgo = sqrt(vx * vx + vy * vy);
                double sEgo = sensor_fusion[i][5];
                sEgo = sEgo + previous_size * 0.02 * vEgo;
                
                if(d < (2 + 4*lane + 2) && d > (2 + 4*lane - 2))
                {
                    if((sEgo > car_s) && ((sEgo - car_s) < 30))
                    {
                        too_close = true;
                    }
                }
                ...
                {
                    if(sEgo < (car_s + 30) && car_s < (sEgo + 30))
                    {
                        car_on_left = true;
                    }
                }
                ...
                // adapt reference speed
                if (too_close)
                {
                    ref_velocity = std::max(0.0, ref_velocity - aEgo);
                ...
                else if (ref_velocity < vMax)
                {
                    ref_velocity = std::min(ref_velocity+aEgo, vMax);
                }
                ...
                // if previous path is less than 2 points, use car location as starting point
                if(previous_size < 2)
                {
                    // calculate the previous pose based on yaw angle
